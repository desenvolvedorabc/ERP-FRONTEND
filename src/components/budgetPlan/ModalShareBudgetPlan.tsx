...
import { shareBudgetPlan } from '@/services/budgetPlan'
...
const userSchema = z.object({
  ...
  budgetPlanId: z.number({ required_error: 'Campo Obrigatório' }),
})
// extracting the type
type ShareBudgetPlan = z.infer<typeof userSchema>

export function ModalShareBudgetPlan({ open, onClose, budgetPlanId }: Props) {
  ...
  const onSubmit: SubmitHandler<ShareBudgetPlan> = async (data, e) => {
    e?.preventDefault()
    setIsDisabled(true)
    ...
    let response
    try {
      response = await shareBudgetPlan({
  ...
  useEffect(() => {
    setValue('budgetPlanId', budgetPlanId)
  }, [budgetPlanId])
    ...
    >
      <Box className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 `}>
          ...
          <form onSubmit={handleSubmit(onSubmit)}>
            <CardContent className="px-0 w-full">
              <div className="font-semibold text-sm mb-2">
                Informe os emails separados por “;” para enviar o link e senha do plano
                orçamentário:
              </div>
        ...
        <ModalConfirm
          ...
          text={success ? 'Plano Orçamentário compartilhado com sucesso!' : errorMessage}
          success={success}
        />
      </Box>
    </Modal>
