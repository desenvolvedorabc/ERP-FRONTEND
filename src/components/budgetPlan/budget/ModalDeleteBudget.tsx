...
export function ModalDeleteBudget({ open, onClose, budgetId, budgetName }: Props) {
  ...
  const onSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    let response
    try {
      response = await deleteBudget(budgetId)
    } catch (err) {
      setIsDisabled(false)
    } finally {
      setIsDisabled(false)
      ...
      <Box className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 `}>
        <Card className="w-96 p-4">
          <CardHeader className="h-16 flex flex-row items-center justify-between border-b border-erp-neutrals mb-5 px-0">
            <div className="text-xl">Excluir Orçamento</div>
          ...
          <form onSubmit={onSubmit}>
            <CardContent className="flex px-0 w-full">
              <div className="w-full text-sm font-semibold mb-2">
                Atenção, você está prestes a excluir o orçamento “{budgetName}”, isso não pode ser
                desfeito. Tem certeza?
              </div>
            </CardContent>
        ...
        <ModalConfirm
          ...
          text={success ? 'Orçamento excluído com sucesso!' : errorMessage}
          success={success}
        />
      </Box>
    </Modal>
